...
def batch_collate_fn(batch):
    ...
    batch[1] = {'patches': patches, 'scale_embedding': scale_embedding.float()}
    return tuple(batch)
...
def pad_ratio(img):
    ...
    new_image = F.pad(img,[pw,ph])

    return new_image


class FSC147Dataset(Dataset):
    def __init__(self, data_dir, data_list, scaling, box_number=3, scale_number=20, min_size=384, max_size=1584, preload=False, main_transform=None, query_transform=None):
        ...
        if file_name in self.images:
            ...
            if points.shape[0] > 0:     
                ...
                for box in boxes:
                    ...
                    # calculate scale
                    scale_embedding.append((abs(x2-x1),abs(y2-y1)))

            scale_embedding = np.array(scale_embedding)
            scale_embedding = scale_embedding.mean(axis=0)
            scale_embedding /= (nw,nh)
...
def build_dataset(cfg, is_train):
    ...
    query_transform = RefTransform(is_train, cfg.DATASET.exemplar_size)
    if is_train: 
...
if __name__ == '__main__':
    ...
    dataset = FSC147Dataset(data_dir='/mnt/storage/oc/fsc_147',
                            data_list='/mnt/storage/oc/fsc_147/train.txt',
                            scaling=1.0,
                            main_transform=main_transform,
                            query_transform=query_transform)
    
    data_loader = DataLoader(dataset, batch_size=5,num_workers=0, collate_fn=batch_collate_fn)
    
    for idx, sample in enumerate(data_loader):
        img, patches, targets = sample
        import ipdb;ipdb.set_trace()
        print(img.size(),patches)
        if idx == 10:
            break
