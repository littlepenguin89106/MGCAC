...
from dataset.FSC147 import build_dataset, batch_collate_fn 
from dataset.train import build_dataset as build_train_dataset, batch_collate_fn as train_collate_fn
...
def main(args):
    ...
    # define dataset
    dataset_train = build_train_dataset(cfg, is_train=True)
    dataset_val = build_dataset(cfg, is_train=False)

    data_loader_train = DataLoader(dataset_train, batch_size=cfg.TRAIN.batch_size, collate_fn=train_collate_fn, shuffle=True, num_workers=cfg.TRAIN.num_workers)
    ...
    if cfg.VAL.evaluate_only:
        ...
        if cfg.VAL.visualization:
            ...
            mae = evaluate(cfg,model, data_loader_val, device, cfg.DIR.output_dir)
        return
    ...
    if os.path.isfile(cfg.TRAIN.resume):
        ...
        if not cfg.VAL.evaluate_only and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            val_nae_list = checkpoint['val_nae']

    best_nae = 10000 if len(val_nae_list) == 0 else min(val_nae_list)
    ...
    for epoch in range(cfg.TRAIN.start_epoch, cfg.TRAIN.epochs):
        ...
        mae, mse,nae,rse = evaluate(cfg,model, data_loader_val, device, cfg.DIR.output_dir)
        loss_list.append(loss)
        val_nae_list.append(nae)
        lr_scheduler.step()
        ...
        utils.plot_learning_curves(loss_list, val_nae_list, cfg.DIR.output_dir)
        
        if cfg.DIR.output_dir:
            with (output_dir / "log.txt").open("a") as f:
                f.write('Epoch %d: loss %.8f, NAE %.2f, RSE %.2f, Best MAE %.2f, Best MSE %.2f, Best NAE %.2f, BSE RSE %.2f\n'%(epoch +1, loss, nae, rse, best_mae, best_mse,best_nae,best_rse))
